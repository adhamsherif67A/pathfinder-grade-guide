...
function DegreePlannerPage() {
  ...
  const commitTerm = async (semester: string) => {
    ...
    const confirm = window.confirm(
      `Move all ${termPlanned.length} courses from Sem ${semester} to your active GPA Calculator?`,
    );
    if (!confirm) return;
    ...
    try {
      ...
      toast.success(`Academic records updated for Sem ${semester}`);
  ...
  const resetPlan = () => {
    if (window.confirm("Erase your entire future roadmap? Your current GPA data is safe.")) {
      setPlannedCourses([]);
  ...
  const filteredCatalog = useMemo(() => {
    return CURRICULUM.filter((c) => {
      ...
      const isAlreadyIn =
        enrolledCodes.has(code) || plannedCourses.some((p) => p.course_code === code);
  ...
  if (loading)
    return (
      <div className="text-center py-20 text-muted-foreground italic animate-pulse">
        Syncing roadmap data...
      </div>
    );
    ...
    return (
      <div className="glass-strong rounded-2xl p-12 text-center max-w-2xl mx-auto mt-20">
        ...
        <p className="text-muted-foreground text-sm mb-6">
          Roadmaps are student-specific journey tools.
        </p>
        <Link to="/advisor">
    ...
    <div className="space-y-6 pb-24 md:pb-20">
      <div className="flex flex-wrap items-end justify-between gap-4 px-1">
        <div className="flex-1 min-w-[200px]">
          <h1 className="text-2xl sm:text-3xl font-black text-gradient tracking-tighter uppercase">
            Degree Planner
          </h1>
          <p className="text-muted-foreground text-xs sm:text-sm font-medium">
            Strategic multi-semester forecasting
          </p>
        </div>
        ...
        <div className="flex w-full sm:w-auto gap-2">
          <Button
            ...
            onClick={resetPlan}
            size="sm"
            className="flex-1 sm:flex-none text-muted-foreground h-11 px-4 rounded-2xl border border-white/5 bg-white/5"
          >
          ...
          <Button
            ...
            size="sm"
            className="flex-1 sm:flex-none h-11 px-6 rounded-2xl border-primary/20 text-primary hover:bg-primary hover:text-primary-foreground gap-2 font-black uppercase tracking-widest text-[10px]"
          >
      ...
      <div className="md:hidden fixed bottom-32 right-6 z-[60]">
        <Button
          size="icon"
          className="h-16 w-16 rounded-full shadow-2xl shadow-primary/50 border-4 border-white/10 bg-primary text-primary-foreground active:scale-90 transition-transform"
          onClick={() => setCatalogOpen(true)}
        >
      ...
      <div className="space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {plannerSemesters.map((semNum) => {
            ...
            const semCompleted = CURRICULUM.filter(
              (c) => c.semester === semNum && passedCodes.has(c.code.toUpperCase()),
            ...
            return (
              <SemesterCol
                ...
                courses={[
                  ...semCompleted,
                  ...semPlanned.map((p) => ({ ...p, isCompleted: false })),
                ]}
        ...
        <div
          className={`glass-strong rounded-[2.5rem] p-8 border-l-8 transition-all duration-700 ${violations.length === 0 ? "border-emerald-500 bg-emerald-500/5" : "border-amber-500 bg-amber-500/5 shadow-2xl shadow-amber-500/10"}`}
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <Badge
              variant="outline"
              className={`font-black rounded-full px-4 ${violations.length === 0 ? "text-emerald-500 border-emerald-500/20" : "text-amber-500 border-amber-500/20"}`}
            >
          ...
          {violations.length === 0 ? (
            <p className="text-sm font-medium text-muted-foreground italic">
              Your academic forecast aligns with all prerequisite protocols.
            </p>
          ) : (
            <div className="grid sm:grid-cols-2 gap-4">
              {violations.map((v, idx) => (
                <div
                  key={idx}
                  className="glass rounded-[1.5rem] p-5 border border-amber-500/20 bg-black/20 flex gap-4 backdrop-blur-sm"
                >
                  <ArrowRightCircle className="h-6 w-6 text-amber-500 shrink-0 mt-1" />
                  <div>
                    <div className="font-black text-xs text-amber-200 uppercase tracking-tight mb-1">
                      {v.course_code} PREREQ CONFLICT
                    </div>
          ...
          <div className="p-4 sm:p-10 flex flex-col h-full min-h-0">
            <DialogHeader className="mb-8 shrink-0">
              <div className="flex items-center justify-between">
                <DialogTitle className="flex items-center gap-4 text-3xl sm:text-4xl font-black tracking-tighter">
                  <div className="p-3 rounded-[1.5rem] bg-primary/20">
                ...
                <Button
                  ...
                  className="rounded-full h-12 w-12 bg-white/5"
                >
            ...
            <div className="flex-1 overflow-y-auto overflow-x-hidden space-y-5 pr-2 custom-scrollbar pb-32 sm:pb-4 touch-pan-y overscroll-contain">
              {filteredCatalog.length === 0 && (
                <div className="text-center py-24 text-muted-foreground animate-in fade-in slide-in-from-bottom-4">
                  <Search className="h-20 w-16 mx-auto mb-6 opacity-5" />
                  <p className="font-black uppercase tracking-widest text-xs">No subjects found</p>
                </div>
              )}
              {filteredCatalog.map((course) => (
                <div
                  key={course.code}
                  className={`glass-strong rounded-[2.5rem] p-7 border transition-all duration-500 hover:scale-[1.01] ${course.uclan ? "border-l-[12px] border-l-[#FFC000]" : "border-white/10"}`}
                >
                  <div className="mb-8">
                    <div className="flex items-center gap-4 mb-3">
                      <span className="font-mono text-sm text-primary font-black uppercase tracking-[0.2em] bg-primary/10 px-3 py-1 rounded-xl">
                        {course.code}
                      </span>
                      {course.uclan && (
                        <Badge className="bg-[#FFC000] text-black text-[10px] h-6 border-none font-black px-3 rounded-full">
                          UK DUAL CERTIFIED
                        </Badge>
                      )}
                    </div>
                    <h4 className="font-black text-xl sm:text-2xl leading-none tracking-tighter mb-3">
                      {course.name}
                    </h4>
                    <div className="flex flex-wrap gap-2">
                      <span className="px-3 py-1 rounded-full bg-white/5 text-[10px] font-black uppercase tracking-widest opacity-60">
                        {course.credits} Credits
                      </span>
                      <span className="px-3 py-1 rounded-full bg-white/5 text-[10px] font-black uppercase tracking-widest opacity-60">
                        Origin: Sem {course.semester}
                      </span>
                    </div>
                  </div>

                  <div className="space-y-4">
                    <div className="text-[10px] font-black text-primary uppercase tracking-[0.3em] opacity-40 ml-1">
                      Assign to Term:
                    </div>
            ...
            <div className="sm:hidden fixed bottom-6 left-6 right-6 z-[70]">
              <Button
                className="w-full h-16 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] font-black text-sm uppercase tracking-widest border-2 border-white/20 bg-background/80 backdrop-blur-3xl"
                onClick={() => setCatalogOpen(false)}
              >
                Close Catalog
              </Button>
...
function SemesterCol({
  ...
  return (
    <div
      className={`glass-strong rounded-[2rem] p-6 flex flex-col h-full min-h-[200px] border transition-all duration-700 ${isRecommended ? "border-primary shadow-[0_0_50px_rgba(var(--color-primary-rgb),0.2)] scale-[1.02] z-10 bg-primary/[0.02]" : "border-white/5"}`}
    >
      <div className="flex items-center justify-between mb-6 pb-4 border-b border-white/5">
        <h3
          className={`font-black uppercase tracking-widest text-xs flex items-center gap-3 ${isRecommended ? "text-primary" : "text-foreground/40"}`}
        >
          <Calendar className="h-5 w-5" /> Sem {sem}
          {isRecommended && (
            <Badge className="text-[8px] h-4 px-2 bg-primary text-primary-foreground border-none font-black uppercase">
              Active
            </Badge>
          )}
        </h3>
        <div className="text-[11px] font-black text-primary bg-primary/10 px-3 py-1 rounded-xl">
          {courses.reduce(
            (acc, c) => acc + (CURRICULUM_BY_CODE[c.course_code.toUpperCase()]?.credits || 0),
            0,
          )}{" "}
          CR
        </div>
            ...
            return (
              <div
                ...
                className={`glass-strong rounded-[1.5rem] p-5 border transition-all duration-300 ${
                  plan.isCompleted
                    ? "border-emerald-500/20 bg-emerald-500/[0.02] opacity-50 grayscale-[0.8]"
                    : hasViolation
                ...
                <div className="flex justify-between items-start gap-4">
                  <div className="min-w-0">
                    <div className="flex items-center gap-3">
                      ...
                      {plan.isCompleted ? (
                        <Badge className="text-[8px] h-4 px-2 bg-emerald-500/20 text-emerald-400 border-none font-black uppercase">
                          PASSED
                        </Badge>
                      ) : hasViolation ? (
                    ...
                    <div className="text-[13px] font-bold truncate mt-2 tracking-tight">
                      {course?.name}
                    </div>
                  </div>
                  {!plan.isCompleted && (
