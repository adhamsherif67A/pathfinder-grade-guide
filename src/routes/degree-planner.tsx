...
function DegreePlannerPage() {
  ...
  const commitTerm = async (semester: string) => {
    ...
    const confirm = window.confirm(`Move all ${termPlanned.length} courses from Sem ${semester} to your active GPA Calculator?`);
    if (!confirm) return;
    ...
    <div className="space-y-6 pb-24 md:pb-20">
      <div className="flex flex-wrap items-end justify-between gap-4 px-1">
        <div className="flex-1 min-w-[200px]">
          <h1 className="text-2xl sm:text-3xl font-black text-gradient tracking-tighter uppercase">Degree Planner</h1>
          <p className="text-muted-foreground text-xs sm:text-sm font-medium">Strategic multi-semester forecasting</p>
        </div>
